...
async function demographics(){
    let url = window.serviceUri + '/Patient?_id=' + window.patient_id;
    let bundle = await getResource(url);
...
async function get_diagnostic_reports(){
    let url = window.serviceUri + '/DiagnosticReport?subject=' + window.patient_id + '&_count=200';
    let next = true;
    let reports = [];
...
async function get_list_of_tests(){
    let url = window.serviceUri + '/ValueSet?name=arup-genetic-tests';
    let bundle = await getResource(url);
...
async function displaySummary(){
    ...
    document.getElementById('main-content').innerHTML = '<img style="padding-left:350px;" src="../img/loader.gif"/>';
    let report = await getResource(window.serviceUri + '/DiagnosticReport/' + window.reportID);
    //let report =  bundle.entry[0].resource;
    let test = report.code.coding[0].display;
    let practitioner_id = report.performer[0].reference;
    let practitioner = await getResource(window.serviceUri + '/' + practitioner_id);
    let practitioner_name = practitioner.name[0].given[0] + ' ' + practitioner.name[0].family;
    let practitioner_details = '(' + practitioner.identifier[0].value + ', ' + practitioner.identifier[0].system + ')';
    let conclusion = report.conclusion;
    let background_info = '';
...
async function gatherIDs(){
    let r = await getResource(window.serviceUri + '/DiagnosticReport/' + window.reportID);
    ...
    r = await getResource(window.serviceUri + '/Encounter/' + window.clinic_encounter);
    ...
    r = await getResource(window.serviceUri + '/ServiceRequest/' + window.test_request);
    window.lab_location = r.locationReference[0].reference.split('/')[1];
    let bundle = await getResource(window.serviceUri + '/ServiceRequest?encounter=' + window.clinic_encounter);
    ...
    bundle = await getResource(window.serviceUri + '/Encounter?based-on=' + window.blood_request);
    r = bundle.entry[0].resource;
    window.blood_encounter = r.id;
    window.blood_procedure = r.diagnosis[0].condition.reference.split('/')[1];
    bundle = await getResource(window.serviceUri + '/Procedure?based-on=' + window.test_request);
    r = bundle.entry[0].resource;
...
async function viewResource(li, type) {
    ...
    switch (type) {
        case 'pat':
            url = window.serviceUri + '/Patient/' + window.patient_id;
            r = await getResource(url);
            document.getElementById('view').innerText = JSON.stringify(r, null, 2);
            break;

        case 'pract-clinic':
            url = window.serviceUri + '/Practitioner/' + window.clinic_practitioner;
            r = await getResource(url);
            document.getElementById('view').innerText = JSON.stringify(r, null, 2);
            break;

        case 'loc-clinic':
            url = window.serviceUri + '/Location/' + window.clinic_location;
            r = await getResource(url);
            document.getElementById('view').innerText = JSON.stringify(r, null, 2);
            break;

        case 'enc-clinic':
            url = window.serviceUri + '/Encounter/' + window.clinic_encounter;
            r = await getResource(url);
            document.getElementById('view').innerText = JSON.stringify(r, null, 2);
            break;

        case 'sr-blood':
            url = window.serviceUri + '/ServiceRequest/' + window.blood_request;
            r = await getResource(url);
            document.getElementById('view').innerText = JSON.stringify(r, null, 2);
            break;

        case 'sr-test':
            url = window.serviceUri + '/ServiceRequest/' + window.test_request;
            r = await getResource(url);
            document.getElementById('view').innerText = JSON.stringify(r, null, 2);
            break;

        case 'pract-blood':
            url = window.serviceUri + '/Practitioner/' + window.blood_practitioner;
            r = await getResource(url);
            document.getElementById('view').innerText = JSON.stringify(r, null, 2);
            break;

        case 'enc-blood':
            url = window.serviceUri + '/Encounter/' + window.blood_encounter;
            r = await getResource(url);
            document.getElementById('view').innerText = JSON.stringify(r, null, 2);
            break;

        case 'proc-blood':
            url = window.serviceUri + '/Procedure/' + window.blood_procedure;
            r = await getResource(url);
            document.getElementById('view').innerText = JSON.stringify(r, null, 2);
            break;

        case 'specimen':
            url = window.serviceUri + '/Specimen/' + window.specimen;
            r = await getResource(url);
            document.getElementById('view').innerText = JSON.stringify(r, null, 2);
            break;

        case 'loc-lab':
            url = window.serviceUri + '/Location/' + window.lab_location;
            r = await getResource(url);
            document.getElementById('view').innerText = JSON.stringify(r, null, 2);
            break;

        case 'pract-lab':
            url = window.serviceUri + '/Practitioner/' + window.lab_practitioner;
            r = await getResource(url);
            document.getElementById('view').innerText = JSON.stringify(r, null, 2);
            break;

        case 'proc-test':
            url = window.serviceUri + '/Procedure/' + window.test_procedure;
            r = await getResource(url);
            document.getElementById('view').innerText = JSON.stringify(r, null, 2);
            break;

        case 'obs-imp':
            url = window.serviceUri + '/Observation/' + window.res_implication;
            r = await getResource(url);
            document.getElementById('view').innerText = JSON.stringify(r, null, 2);
            break;

        case 'obs-int':
            url = window.serviceUri + '/Observation/' + window.res_interpretation;
            r = await getResource(url);
            document.getElementById('view').innerText = JSON.stringify(r, null, 2);
            break;

        case 'obs-gen':
            url = window.serviceUri + '/Observation/' + window.res_genotype;
            r = await getResource(url);
            document.getElementById('view').innerText = JSON.stringify(r, null, 2);
            break;

        case 'obs-var':
            url = window.serviceUri + '/Observation/' + window.res_variant;
            r = await getResource(url);
            document.getElementById('view').innerText = JSON.stringify(r, null, 2);
            break;

        case 'dr':
            url = window.serviceUri + '/DiagnosticReport/' + window.reportID;
            r = await getResource(url);
            document.getElementById('view').innerText = JSON.stringify(r, null, 2);
            break;
