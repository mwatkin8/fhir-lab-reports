...
async function fetchTemplate(type){
    let response = await fetch('../resource-templates/' + type + '.json');
...
async function handleHookService(server){
    ...
    let response = await fetch(server + 'PlanDefinition?url=fhir_lab_reports_genetic_service');
    let bundle = await response.json();
    if (bundle.total !== 0){
        return bundle.entry[0].resource.id;
    }
    ...
    t.url = 'fhir_lab_reports_genetic_service';
    t.title = 'Hooks service endpoint for the fhir-lab-reports-genetic project';
...
async function createHookResponse(patient, diag_report, res_implication, server){
    ...
    t.code.coding[0].code = 'fhir_lab_reports_genetic_response';
    t.subject.reference += patient;
